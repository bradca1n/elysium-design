    ...
    <div style={{padding:'48px 40px 80px',maxWidth:1160,margin:'0 auto'}} data-page>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:24,marginBottom:32}}>
        <div style={{display:'flex',alignItems:'center',gap:8,fontSize:14,color:'var(--ink-2)'}}>
          ...
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{color:'var(--ink-3)'}}><polyline points="9 6 15 12 9 18"/></svg>
          <span style={{color:'var(--ink-1)',fontWeight:500}}>NAV</span>
        </div>
        <button aria-label="More" style={{width:32,height:32,border:'none',background:'transparent',borderRadius:8,cursor:'pointer',color:'var(--ink-2)',display:'inline-flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>e.currentTarget.style.background='var(--bg-subtle)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
      ...
      <section style={{marginBottom:56}}>
        <SectionHead title="NAV" desc="Portfolio allocation and live market values across custody & exchanges."/>
          ...
          <div>
            ...
            {assets.map(a => (
              <div key={a.id} onMouseEnter={() => setActiveAsset(a.id)} onMouseLeave={() => setActiveAsset(null)} style={{
                ...
                background: activeAsset === a.id ? 'var(--bg-subtle)' : 'transparent',
      ...
      <section style={{marginBottom:56}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,marginBottom:28}} data-kpi-grid>
        ...
        <div style={{marginTop:4}}>
          ...
          {/* Closing row */}
          <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 160px 200px',gap:20,alignItems:'center',padding:'14px 4px',fontSize:13,fontVariantNumeric:'tabular-nums',background:'var(--bg-subtle)',borderRadius:8,marginTop:4,paddingLeft:16,paddingRight:16}}>
      ...
      <section>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:28}} data-kpi-grid>
        ...
        <div style={{border:'1px solid var(--line-1)',borderRadius:10,overflow:'hidden'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 20px',background:'#15151A',color:'#F2F0EC'}}>
...
function SectionHead({ title, desc, cta }) {
  return (
...
function BSSection({ title, rows, total }) {
  return (
    <>
      <div style={{padding:'10px 20px',background:'var(--bg-subtle)',fontSize:10.5,fontWeight:600,color:'var(--ink-2)',borderBottom:'1px solid var(--line-1)'}}>{title}</div>
      {rows.map((r,i) => (
